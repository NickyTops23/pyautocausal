...
class AutoCleaner:
    ...
    def clean(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, CleaningMetadata]:
        """
        Runs the full validation, planning, and cleaning pipeline.
        ...
        Returns:
            ...
            - A CleaningMetadata object with details of the operations performed.
            
        Note: This method is kept for backward compatibility. For new code,
        prefer clean_with_logging() which logs metadata instead of returning it.
        """
        # 1. Validate
    ...
    def clean_with_logging(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Runs the full validation, planning, and cleaning pipeline with metadata logging.

        Args:
            df (pd.DataFrame): The input DataFrame to clean.

        Returns:
            The cleaned DataFrame. Metadata is logged using Python's logging system.
        """
        logger = logging.getLogger(f"{__name__}.AutoCleaner")
        
        # Run the same cleaning logic as the original method
        cleaned_df, metadata = self.clean(df)
