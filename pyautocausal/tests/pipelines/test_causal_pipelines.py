...
def test_unified_causal_pipeline(tmp_path):
    """Test that the unified causal pipeline (with both cross-sectional and panel paths) executes successfully."""
    # Create output directory
    output_path = tmp_path / "unified_output"
    output_path.mkdir(parents=True, exist_ok=True)

    # Initialize the unified graph
    graph = causal_pipeline(output_path)

    # Generate mock data with multiple periods (triggers panel path)
...
def test_panel_data_pipeline(tmp_path):
    """Test the panel data pipeline with multi-period data."""
    # Create output directory
    output_path = tmp_path / "panel_output"
    output_path.mkdir(parents=True, exist_ok=True)
    ...
    graph = create_panel_graph(abs_text_dir, abs_plots_dir)
    graph.configure_runtime(output_path=output_path)

    # Generate mock data with multiple periods (standard DiD scenario)
    ...
    # Check that appropriate panel data files were generated
...
def test_cross_sectional_pipeline(tmp_path):
    """Test the cross-sectional pipeline with single-period data."""
    # Create output directory
    ...
    # Initialize the cross-sectional graph
    graph = create_cross_sectional_graph(abs_text_dir, abs_plots_dir)
    graph.configure_runtime(output_path=output_path)

    # Generate mock data with single period (cross-sectional scenario)
    ...
    # Check that appropriate cross-sectional files were generated
    text_files = list((output_path / "text").glob("*.txt"))
...
def test_serialized_simple_graph_execution(tmp_path):
    ...
    # 4. Execute the loaded graph with mock data (single period for cross-sectional analysis)
    mock_data = generate_mock_data(n_units=50, n_periods=1, n_treated=10)
    ...
    #    The mock data has single period, so the cross-sectional branch should run.
    stand_spec_node = loaded_graph.get("stand_spec")
    ols_stand_node = loaded_graph.get("ols_stand")
    ols_stand_output_node = loaded_graph.get("ols_stand_output")

    # The cross-sectional branches should be COMPLETED
    assert stand_spec_node.state == NodeState.COMPLETED, "Cross-sectional spec node should have completed."
    assert ols_stand_node.state == NodeState.COMPLETED, "Cross-sectional OLS node should have completed."
    assert ols_stand_output_node.state == NodeState.COMPLETED, "Cross-sectional output node should have completed."
    
    # Note: Output file check not needed since this is a serialization test with hardcoded paths in simple_graph
